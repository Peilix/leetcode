    ...
    while (!q.empty() && fresh != 0) {
        ...
        while (size-- != 0) {
            ...
            if (0 < rotted_coord.first)
                rotting_coords.push_back(std::make_pair(rotted_coord.first - 1, rotted_coord.second));
            if (rotted_coord.first + 1 < grid.size())
                rotting_coords.push_back(std::make_pair(rotted_coord.first + 1, rotted_coord.second));
            if (0 < rotted_coord.second)
                rotting_coords.push_back(std::make_pair(rotted_coord.first, rotted_coord.second - 1));
...
int Solution::maxProfit_4th(int k, std::vector<int>& prices)
{
    ...
    while (right < prices.size()) {
        if (prices[left] < prices[right]) {
            while (right + 1 < prices.size() && prices[right] <= prices[right + 1]) {
                right++;
            }
...
void Solution::nextPermutation(std::vector<int>& nums)
{
    ...
    for (;;) { // find rightmost element smaller than successor
        auto _UNext1 = _UNext;
        if (*--_UNext < *_UNext1) { // swap with rightmost element that's smaller, flip suffix
            auto _UMid = _ULast;
            do {
                --_UMid;
            } while (!(*_UNext < *_UMid));
...
{
    if (obstacleGrid.empty() || obstacleGrid[0].empty() || 1 == obstacleGrid.back().back())
        return 0;
    ...
    for (int row = obstacleGrid.size() - 2; row >= 0; row--) {
        if (1 == obstacleGrid[row][obstacleGrid[0].size() - 1])
            continue;
        dp[row][obstacleGrid[0].size() - 1] = dp[row + 1][obstacleGrid[0].size() - 1];
    }
    for (int column = obstacleGrid.size() - 2; column >= 0; column--) {
        if (1 == obstacleGrid[obstacleGrid.size() - 1][column])
            continue;
        dp[obstacleGrid.size() - 1][column] = dp[obstacleGrid.size() - 1][column + 1];
    }
    for (int row = obstacleGrid.size() - 2; row >= 0; row--) {
        for (int column = obstacleGrid[0].size() - 2; column >= 0; column--) {
...
int partition(std::vector<int>& nums, size_t first, size_t last)
{
#if true
...
{
    if (first < last) {
        size_t pivot_location = partition(nums, first, last);
...
{
    auto findAncestors = [&](TreeNode* root, TreeNode* p, std::vector<TreeNode*> ancestors) {
        if (root == p) {
        }
    };
    return nullptr;
...
int Solution::minOperationsMaxProfit(std::vector<int>& customers, int boardingCost, int runningCost)
{
    ...
    for (size_t i = 0; i < customers.size(); i++) {
        ...
        total -= board;
        if (0 < board) {
            profits.push_back(profited + board * boardingCost - runningCost);
...
std::vector<int> Solution::findRedundantConnection(std::vector<std::vector<int>>& edges)
{
    ...
    size_t i = 0;
    auto find = [&](int p) -> int {while (p != root[p]) p = root[p]; return p; };
    for (; i < edges.size(); i++) {
...
// TODO:
std::vector<int> Solution::findRedundantDirectedConnection(std::vector<std::vector<int>>& edges)
{
    std::vector<int> root(edges.size() + 1);
    for (size_t i = 0; i < root.size(); i++) {
        root[i] = i;
...
std::vector<int> Solution::asteroidCollision(std::vector<int>& asteroids)
{
    ...
    for (auto asteroid : asteroids) {
        if (stk.empty()) {
            ...
            // collision
            while (!stk.empty() && 0 < stk.back() && asteroid + stk.back() < 0)
                stk.pop_back();
